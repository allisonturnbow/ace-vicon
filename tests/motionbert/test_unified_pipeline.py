...
def _fake_pose_3d(frames: int) -> np.ndarray:
...
def test_motionbert_stage_returns_marker_dict(tmp_path):
    ...
    runner.generate_3d_pose = lambda *_args, **_kwargs: _fake_pose_3d(poses.shape[0])
    try:
        markers = run_motionbert_stage(tmp_path, backend="geometric")
    finally:
        runner.generate_3d_pose = original

    assert "frames" in markers
    assert "right_hand" in markers
    assert len(markers["frames"]) == poses.shape[0]
    assert (tmp_path / "ace_markers.npz").is_file()


def test_motionbert_markers_work_with_segmentation(tmp_path):
    poses, confidence = _poses_2d(frames=120)
    np.save(tmp_path / "poses_2d.npy", poses)
    np.save(tmp_path / "poses_2d_confidence.npy", confidence)
    (tmp_path / "video_metadata.json").write_text(
        json.dumps(
    ...
    original = runner.generate_3d_pose
    runner.generate_3d_pose = lambda *_args, **_kwargs: _fake_pose_3d(120)
    try:
        markers = run_motionbert_stage(tmp_path, backend="geometric")
    finally:
        runner.generate_3d_pose = original

    result = segment_serve(markers, SegmentationConfig(use_legacy_detection=False))
