"""Reusable Velocity Kernel for angular-velocity / deceleration scoring.

Mathematics
-----------
Given a 1D series ``v(t)`` (deg/s for velocity features, deg/s² for
deceleration magnitude) and a phase window, the kernel finds the peak
``P`` (max by default) and compares it to the reference peak ``R``.

**Magnitude** (weight 0.50)::

    tolerance = max(min_tolerance, 0.15 · |R|)
    magnitude_score = 100 · exp(−½ · ((P − R) / tolerance)²)

Direction uses signed error ``P − R`` with a 5%·tolerance dead-zone
(``too_low`` / ``too_high`` / ``acceptable``). Higher series values mean
“more” of the coached quantity (faster rotation, stronger deceleration).

**Timing** (weight 0.20)::

    τ = peak_index / max(n_window − 1, 1)   # ∈ [0, 1]
    timing_score = 100 · exp(−½ · ((τ_p − τ_r) / 0.15)²)

**Smoothness** (weight 0.15)::

    jerk_rms = √(mean(Δ² v)²)
    ratio = jerk_rms_player / max(jerk_rms_reference, ε)
    smoothness_score = 100 if ratio ≤ 1 else Gaussian(ratio − 1, tol=0.5)

**Consistency** (weight 0.15)::

    Multi-serve CV of trial peaks → Gaussian(cv, tol=0.08);
    else neutral 75 (``single_serve_neutral``).

**Confidence**
    finite coverage × mean joint confidence × phase-window length factor.
    Future: MotionBERT confidence, ``marker_validity_mask``, incomplete-phase flags.

**Scalar fallback**
    Magnitude only from scalar peaks; timing/smoothness = 75; confidence = 0.55.

Assumptions
-----------
- Phase windows are comparable after segmentation / DTW.
- Callers supply a series where larger values = more of the coached quality.
- fps is known when differentiating angle → velocity → deceleration.
"""
...
def peak_and_timing(
    ...
    *,
    ...
    if not np.any(finite):
        raise ValueError("Velocity window has no finite samples")
    filled = window.copy()
    ...
    return peak, local_idx / denom, local_idx
...
def _magnitude_component(
    ...
    *,
    ...
    tolerance = max(float(min_tolerance), _MAGNITUDE_RELATIVE_TOLERANCE * abs(reference_peak))
    ...
    return score, direction, {
        f"player_peak_{peak_label}": player_peak,
        f"reference_peak_{peak_label}": reference_peak,
        f"peak_{peak_label}_error": round(error, 4),
...
def _consistency_component(player_trial_peaks: Sequence[float] | None) -> tuple[float, dict]:
    ...
    return score, {
        ...
        "trial_peak_count": int(peaks.size),
        "trial_peak_mean": round(mean, 4),
        "trial_peak_std": round(std, 4),
...
def _confidence_from_quality(
    ...
    *,
    ...
    if n < 8:
        ...
        length_factor = 1.0
    conf = float(_clamp(conf * length_factor, 0.0, 1.0))
...
def score_velocity_feature(
    *,
    ...
    unit: str = "deg/s",
    peak_mode: PeakMode = "max",
    peak_label: str = "velocity",
    min_tolerance: float = _DEFAULT_MIN_TOLERANCE,
) -> FeatureScoreResult:
    """Score a velocity / deceleration feature from player/reference trajectories."""
    ...
    builder = (
        ...
        .add_component("magnitude", magnitude_score, DEFAULT_COMPONENT_WEIGHTS["magnitude"])
        .add_component("timing", timing_score, DEFAULT_COMPONENT_WEIGHTS["timing"])
        .add_component("smoothness", smoothness_score, DEFAULT_COMPONENT_WEIGHTS["smoothness"])
        .add_component(
            "consistency", consistency_score, DEFAULT_COMPONENT_WEIGHTS["consistency"]
        )
        ...
        .add_measurement("series_unit", unit)
    )
    for meta in (
        magnitude_meta,
        timing_meta,
...
def score_velocity_feature_from_scalars(
    *,
    feature_name: str,
    phase: str,
    player_value: float,
    ...
    """Scalar-only fallback when velocity trajectories are unavailable."""
    magnitude_score, direction, magnitude_meta = _magnitude_component(
        float(player_value),
    ...
    builder = (
        ...
        .add_component("magnitude", magnitude_score, DEFAULT_COMPONENT_WEIGHTS["magnitude"])
        .add_component("timing", NEUTRAL_COMPONENT_SCORE, DEFAULT_COMPONENT_WEIGHTS["timing"])
        .add_component(
            "smoothness", NEUTRAL_COMPONENT_SCORE, DEFAULT_COMPONENT_WEIGHTS["smoothness"]
        )
        .add_component(
            "consistency",
