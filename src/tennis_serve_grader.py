...
# ─────────────────────────────────────────────
# 9. EXAMPLE — replace with real pipeline output
# ─────────────────────────────────────────────

if __name__ == "__main__":
    # These frame indices come from Max's segmentation output
    # (phase_to_index_range converts Vicon frames → array indices)
    # Example based on the firstserve output seen in the terminal:
    #   Start_Stance (1, 152), Release (153, 388), Loading (389, 408) ...
    dummy_phase_starts = {
        "Start_Stance":  0,
        "Release":       45,
        "Loading":       112,
    ...
    # These come from Biplav/Devyn's angle computation on the customer video
    ...
    # Load barycenter from default path (dtw/barycenter2.npy)
    # For testing without the full repo, pass a path explicitly:
    # results = grade_serve(dummy_phase_starts, dummy_customer_angles,
    #                       pro_barycenter="C:/path/to/barycenter2.npy")
    results = grade_serve(dummy_phase_starts, dummy_customer_angles)
